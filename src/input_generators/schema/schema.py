"""Parse a JSON input schema and generate synthetic data.

Main entry: ``generate_data_from_schema``. Flow: parse → dependency graph →
topological sort → generate (Variable / List / Matrix / Group) → format.
Supports nested Variable(Group) and List(Group); Matrix(Group) and Group(Group)
are not supported. Limited robustness for very deep nesting.
"""
...
def biased_random_int(min_val: int, max_val: int) -> int:
    """Random int biased toward the outer ~10% of [min_val, max_val] when enabled."""
    if min_val == max_val:
        return min_val
...
def biased_random_float(min_val: float, max_val: float) -> float:
    """Random float biased toward the outer ~10% of the range when enabled."""
    if min_val == max_val:
        return min_val
...
def parse_schema(schema_json: str) -> Dict[str, Any]:
    """Parse schema JSON (strips optional markdown fences) and validate top-level fields."""
    try:
        ...
        for field in required_fields:
            if field not in schema:
                raise ValueError(f"lack of necessary field: {field}")
        ...
        raise ValueError(f"Schema parsing error: {e}")


def extract_dependencies(schema: Dict[str, Any]) -> Dict[str, List[str]]:
    """Top-level parameter dependency graph: key is depended-on name, values are dependents."""
...
def topological_sort(dependency_graph: Dict[str, List[str]]) -> List[str]:
    ...
    if len(result) != len(dependency_graph):
        raise ValueError("cycle dependency detected, cannot perform topological sort")

    return result


def evaluate_expression(expr: str, context: Dict[str, Any]) -> int:
    """Substitute $var / $var.length, then eval with a restricted character set and min/max only."""
        ...
        if '.' in full_match:
            ...
            if property_name == 'length':
                if isinstance(value, (list, tuple, str)):
                    ...
                    raise ValueError(f"variable '{var_name}' does not support length property")
            else:
    ...
    try:
        ...
        for word in words:
            if word.lower() not in allowed_functions and word not in allowed_functions:
                raise ValueError(f"expression contains disallowed functions or identifiers: '{word}'")
        ...
        raise ValueError(f"expression calculation failed: '{expr}' -> '{expr_with_values}', error: {str(e)}")


def generate_variable(param_spec: Dict[str, Any], context: Dict[str, Any]) -> Any:
    """Sample a Variable: int, float, char, or string from range / enum."""
    ...
    if data_type_lower == "int":
        if isinstance(range_spec, list) and len(range_spec) == 2:
            ...
            print(f"warning: int type range_spec format is incorrect: {range_spec}, using default range")
        ...
        if isinstance(range_spec, list) and len(range_spec) == 2:
            ...
            print(f"warning: float type range_spec format is incorrect: {range_spec}, using default range")
        ...
        if isinstance(range_spec, list):
            ...
            print(f"warning: char type range_spec format is incorrect: {range_spec}, using default lowercase character")
        ...
        if isinstance(range_spec, list):
            ...
            print(f"warning: string type range_spec format is incorrect: {range_spec}, using default character 0")
            return "0"

    else:
        raise ValueError(f"unknown data type: '{data_type}', parameter specification: {param_spec}")


def generate_list(param_spec: Dict[str, Any], context: Dict[str, Any]) -> Tuple[List[Any], str]:
    """Build a list and its split character; returns (data, split_char)."""
    data_type = param_spec.get("data_type")
    range_spec = param_spec.get("range")
    ...
    if isinstance(range_spec, list) and len(range_spec) == 2:
        ...
        print(f"warning: List type range_spec format is incorrect: {range_spec}, using default length")
...
def generate_matrix(param_spec: Dict[str, Any], context: Dict[str, Any]) -> Tuple[List[List[Any]], str, str]:
    """Build a matrix; returns (data, row_split, column_split). Row/column limits share one config range."""
    data_type = param_spec.get("data_type")
    ...
    if isinstance(row_range, list) and len(row_range) == 2:
        ...
        print(f"warning: Matrix type row_range format is incorrect: {row_range}, using default number of rows")
    ...
    if isinstance(column_range, list) and len(column_range) == 2:
        ...
        print(f"warning: Matrix type column_range format is incorrect: {column_range}, using default number of columns")
...
def generate_group(param_spec: Dict[str, Any], context: Dict[str, Any]) -> Tuple[Dict[str, Any], List[Dict[str, str]]]:
    """Generate nested parameters with internal topo order; returns (group_data, output_order)."""
...
def generate_parameter(param_name: str, param_spec: Dict[str, Any], context: Dict[str, Any]) -> Union[Any, Tuple[Any, str], Tuple[Any, str, str], Tuple[Any, List[Dict[str, str]]]]:
    """Dispatch by ``type``; updates ``context`` with the value (data only for List/Matrix, not Group)."""
    param_type = param_spec.get("type")
    if not param_type:
        raise ValueError(f"parameter specification lacks type field: {param_spec}")
    ...
    if param_type_lower == "variable":
        ...
        raise ValueError(f"unsupported parameter type: '{param_type}', parameter specification: {param_spec}")
...
def format_output(schema: Dict[str, Any], generated_data: Dict[str, Any]) -> str:
    """Join generated values per ``output_order``; ``function_args`` vs ``stdin``."""
...
def format_function_args(output_order: List[Dict[str, str]], generated_data: Dict[str, Any], schema: Dict[str, Any] = None) -> str:
    """One parameter per line; nested Group as top-level tuple is rejected."""
    result_parts = []
            ...
            if isinstance(data, tuple):
                if len(data) == 2:
                    if isinstance(data[1], str):
                        ...
                        raise ValueError(f"function_args does not support Group type parameters: {param_name}")
                elif len(data) == 3:
...
def format_stdin(output_order: List[Dict[str, str]], generated_data: Dict[str, Any]) -> str:
    """Concatenate values with per-field ``separator`` (unicode escapes resolved)."""
    result_parts = []
...
def format_variable_for_function(data: Any, schema: Dict[str, Any], param_name: str) -> str:
    """Format a Variable for function_args (one line per parameter)."""
...
def format_list_for_function(data: List[Any], split_char: str, schema: Dict[str, Any] = None, param_name: str = "") -> str:
    """List as function arg; handles List(Group), List(List), and string elements (best-effort, ~2D nesting)."""
    if split_char == "":
...
def format_list_for_stdin(data: List[Any], split_char: str) -> str:
    """List for stdin; List(Group) concatenates groups without inter-group split_char."""
    if split_char:
...
def format_matrix_for_function(data: List[List[Any]], row_split: str, column_split: str) -> str:
    """Matrix as function arg: repr of nested list."""
    return str(data)


def format_matrix_for_stdin(data: List[List[Any]], row_split: str, column_split: str) -> str:
    """Matrix rows/columns joined with escaped row_split / column_split."""
    if row_split:
...
def format_group_for_function(data: Dict[str, Any], output_order: List[Dict[str, str]]) -> str:
    """Group as function args: one line per field in output_order."""
    result_parts = []
    for output_item in output_order:
        param_name = output_item["name"]
...
def format_group_as_list(data: Dict[str, Any], output_order: List[Dict[str, str]]) -> List[Any]:
    """Flatten group fields to a list in output_order (for List(Group) in function_args)."""
    result = []
    for output_item in output_order:
        param_name = output_item["name"]
        if param_name in data:
...
def format_group_for_stdin(data: Dict[str, Any], output_order: List[Dict[str, str]]) -> str:
    """Nested stdin formatting inside a group."""
    result_parts = []
    for output_item in output_order:
        param_name = output_item["name"]
        separator = output_item.get("separator", "")
...
def generate_data_from_schema(schema_json: str) -> str:
    """End-to-end: parse, topo-generate, format. List(Group) also registers ``{name}_i`` entries."""
