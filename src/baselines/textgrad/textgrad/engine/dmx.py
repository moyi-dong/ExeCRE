try:
    ...
    raise ImportError("If you'd like to use DMX models, please install the openai package by running `pip install openai`, and add 'DMX_API_KEY' to your environment variables.")
...
# Default base URL for DMX
DMX_BASE_URL = 'https://www.dmxapi.cn/v1'

class ChatDMX(EngineLM, CachedEngine):
    DEFAULT_SYSTEM_PROMPT = "You are a helpful, creative, and smart assistant."

    def __init__(
        self,
        model_string: str="qwen-turbo",
        ...
        **kwargs):
        ...
        cache_path = os.path.join(root, f"cache_dmx_{model_string}.db")

        super().__init__(cache_path=cache_path)

        self.system_prompt = system_prompt
        
        if os.getenv("DMX_API_KEY") is None:
            raise ValueError("Please set the DMX_API_KEY environment variable if you'd like to use DMX models.")
        ...
        self.cache.clear() # delete if need to test same query multiple times
    
    @retry(wait=wait_random_exponential(min=1, max=5), stop=stop_after_attempt(5))
    def generate_completion(self, messages):
        """Chat completion from full ``messages`` list (OpenAI-style)."""
        cache_key = json.dumps(messages)
        cache_or_none = self._check_cache(cache_key)
        if cache_or_none is not None:
            return cache_or_none
        print("[ChatDMX] generate_completion: non-cached path (unexpected in some setups)")
