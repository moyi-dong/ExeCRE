try:
    ...
    raise ImportError(
        "If you'd like to use OpenRouter models, please install the openai package by running `pip install openai`, "
        "and add 'OPENROUTER_API_KEY' to your environment variables."
    )
...
class ChatOpenRouter(EngineLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if os.getenv("OPENROUTER_API_KEY") is None:
            raise ValueError(
                "Please set the OPENROUTER_API_KEY environment variable if you'd like to use OpenRouter models."
            )
        ...
        if isinstance(content, str):
            ...
            if has_multimodal_input and not self.is_multimodal:
                raise NotImplementedError(
                    "Multimodal generation is only supported for models that support it."
                )
    ...
    def _format_content(
        self, content: List[Union[str, bytes]]
    ) -> List[dict]:
        formatted_content = []
        for item in content:
